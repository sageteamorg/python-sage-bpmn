...
from sage_bpmn.helpers.enums import TaskType
...
class BPMNQueryEngine:
    ...
    def get_tasks_by_type(self, task_type: TaskType) -> List[BPMNTask]:
        return [task for task in self.repository.get_tasks().values() if task.task_type == task_type]

    def search_tasks_by_name(self, name_query: str, exact_match: bool = False) -> List[BPMNTask]:
        if exact_match:
            return [task for task in self.repository.get_tasks().values() if task.name == name_query]
        return [task for task in self.repository.get_tasks().values() if name_query.lower() in task.name.lower()]
